module AuthenticatedSystem
  protected
    ...
    # behavior in case the cruser is not authorized
    ...
    # Inclusion hook to make #current_cruser and #logged_in?
    ...
    # Called from #current_cruser.  First attempt to login by the cruser id stored in the session.
    def login_from_session
      self.current_person = Person.find_by_id(session[:person_id]) if session[:person_id]
    end

    # Called from #current_cruser.  Now, attempt to login by basic authentication information.
    def login_from_basic_auth
      authenticate_with_http_basic do |nickname, password|
        self.current_person = Person.authenticate(nickname, password)
      end
    end
    ...
    # Called from #current_cruser.  Finaly, attempt to login by an expiring token in the cookie.
    # for the paranoid: we _should_ be storing cruser_token = hash(cookie_token, request IP)
